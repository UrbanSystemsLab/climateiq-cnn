# -*- coding: utf-8 -*-
"""Preprocess_h3cells.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/16GilHDGiE-PKQN25Ff3xCtpYJDAj-mzH

Inputs are read directly from Google Cloud Storage by default (see
DEFAULT_WORKING_DIR and DEFAULT_H3_ROOT); local directories are accepted too.
Rasters are read window-by-window over the network, so the multi-GB per-cell
DEMs are never downloaded in full. Vectors are read with a bounding-box filter
so only features near the city are loaded. Results are written locally under
--output-dir and, when --destination is given, staged under <destination>/<City>/
in the layout used by gs://citycat-preprocessing/BETA_10_CITIES (DEM/, Buildings/,
Soil/, Green_Spaces/, City_Boundary/, ...). Cities already staged are skipped, so the
script can be re-run over a growing city list and only does new work.

Reading gs:// paths needs Google credentials for GDAL: either run
...
REQUIRED_STAGED_KEYS = ["dem", "buildings", "soil", "green_spaces_polygons", "city_boundary"]
...
def gdal_path(path):
    """Path in a form GDAL (rasterio / geopandas) can open: local, or /vsigs/ for GCS."""
    if is_gcs(path):
...
def configure_gdal_gcs_access():
    """Make /vsigs/ reads work by pointing GDAL at gcloud's application default credentials."""
...
def union_all(geoseries):
    """Union of all geometries; geopandas >= 1.0 has union_all, older versions unary_union."""
...
def copy_file(local_file, destination_path):
    ...
    if is_gcs(destination_path):
        ...
        get_storage_client().bucket(bucket).blob(key).upload_from_filename(str(local_file))
    else:
...
def staged_path(destination, city_name, key):
    ...
    return join_path(destination, city_name, subfolder, name.format(city=safe_name(city_name)))

def staged_files_present(destination, city_name):
    """True if every required staged output for the city already exists at the destination."""
    return all(path_exists(staged_path(destination, city_name, key)) for key in REQUIRED_STAGED_KEYS)
...
def validate_target_crs(target_crs):
    ...
    if target_crs not in {"EPSG:4326", "EPSG:3395"}:
        raise ValueError(f"TARGET_CRS must be EPSG:4326 or EPSG:3395. Received: {target_crs}")
    return target_crs

def select_city(urban_shp, country_name, city_name, country_field="CNTRY_NAME", city_field="JRC_NAME_M"):
    ...
    country_mask = urban[country_field].fillna("").astype(str).str.strip().str.casefold() == country_name.strip().casefold()
    city_mask = urban[city_field].fillna("").astype(str).str.strip().str.casefold() == city_name.strip().casefold()
    selected_city = urban[country_mask & city_mask].copy()

    if selected_city.empty:
        available = urban.loc[country_mask, city_field].dropna().astype(str).sort_values().unique()
        print(f"\nCity '{city_name}' was not found in '{country_name}'.")
...
def find_intersecting_h3(h3_shp, selected_city, h3_field="h3_index"):
    ...
    h3_indexes = selected_h3[h3_field].dropna().astype(str).str.strip().drop_duplicates().sort_values().tolist()
...
def create_h3_city_masks(selected_city, selected_h3, h3_field="h3_index"):
    ...
    for _, row in h3_local.iterrows():
        ...
        masks[h3_index] = gpd.GeoDataFrame({h3_field: [h3_index]}, geometry=[intersection], crs=selected_city.crs)
...
def collect_h3_inputs(h3_root, h3_indexes):
    ...
    for h3_index in h3_indexes:
        ...
        expected = {
            ...
            "green_spaces": join_path(h3_dir, "Green_spaces", f"green_spaces_{h3_index}.tif"),
    ...
    if missing:
        ...
        raise FileNotFoundError("Processing stopped because required H3 datasets are missing.")

    return datasets

def extract_raster_by_mask(input_raster, mask_gdf, output_raster, target_crs, resolution=None, resampling=Resampling.nearest):
    ...
    with rasterio.open(gdal_path(input_raster)) as src:
        ...
        geometries = [mapping(geom) for geom in mask_target.geometry if geom is not None and not geom.is_empty]
        ...
        with WarpedVRT(src, **vrt_options) as vrt:
            out_image, out_transform = mask(vrt, geometries, crop=True, filled=True, nodata=vrt.nodata)
            profile = vrt.profile.copy()
            profile.update(driver="GTiff", height=out_image.shape[1], width=out_image.shape[2], transform=out_transform, crs=target_crs_obj, compress="LZW", tiled=True, BIGTIFF="IF_SAFER")
...
def mosaic_raster_parts(raster_parts, output_raster, target_resolution=None, resampling=Resampling.nearest):
    ...
    try:
        merge_kwargs = {
            ...
            "dst_kwds": {"driver": "GTiff", "compress": "LZW", "tiled": True, "BIGTIFF": "IF_SAFER"}
        }
...
def save_reference_outputs(selected_city, selected_h3, h3_indexes, output_dir, location_name, target_crs):
...
def process_city_data(country_name, city_name, target_crs=DEFAULT_TARGET_CRS, working_dir=DEFAULT_WORKING_DIR, h3_root=DEFAULT_H3_ROOT, output_dir=DEFAULT_OUTPUT_DIR):
    """
    ...
    written to <output_dir>/<Country>_<City>/ on the local filesystem.

    EPSG:4326:
        WGS 84 geographic coordinates; source/native geographic raster resolution retained.

    EPSG:3395:
        WGS 84 / World Mercator; DEM = 2 m, Green spaces = 30 m, Landcover = 30 m.
    """
    ...
    urban_shp = join_path(working_dir, "01_urban_areas_simplified.shp")
    ...
    location_name = f"{safe_name(country_name)}_{safe_name(city_name)}"
    ...
    selected_city = select_city(urban_shp, country_name, city_name)
    ...
    h3_list_file, city_file, h3_file = save_reference_outputs(selected_city, selected_h3, h3_indexes, output_dir, location_name, target_crs)
    ...
    for number, h3_index in enumerate(h3_indexes, start=1):
        ...
        extract_raster_by_mask(source["dem"], mask_gdf, dem_part, target_crs, dem_resolution, Resampling.bilinear)
        dem_parts.append(dem_part)

        print("\nBuildings")
        building_part = vector_temp / f"Buildings_{h3_index}.gpkg"
        if clip_vector_by_mask(source["buildings"], mask_gdf, building_part, target_crs):
            building_parts.append(building_part)

        print("\nGreen spaces")
        green_part = raster_temp / f"green_spaces_{h3_index}.tif"
        extract_raster_by_mask(source["green_spaces"], mask_gdf, green_part, target_crs, green_resolution, Resampling.nearest)
        ...
        extract_raster_by_mask(source["landcover"], mask_gdf, landcover_part, target_crs, landcover_resolution, Resampling.nearest)
    ...
    print("\nLandcover")
    mosaic_raster_parts(landcover_parts, landcover_output, landcover_resolution, Resampling.nearest)
    ...
    if target_crs == "EPSG:3395":
        ...
        print("\nEPSG:4326 selected. Raster resolution remains in geographic coordinates and is not forced to meter-based resolution.")
...
def run_cities(country_name, city_names, target_crs, working_dir, h3_root, output_dir, destination=None, overwrite=False, dry_run=False):
    """Process several cities, skipping ones already staged; one failure does not stop the rest.
    ...
    for city_name in city_names:
        # Staged folders are named by the city exactly as given (matching BETA_10_CITIES);
        # local working outputs use the safe <Country>_<City> name.
        staged_name = city_name.strip()
        print("\n" + "#" * 80)
        print(f"# {country_name} / {city_name}")
        print("#" * 80)

        try:
            if destination and not overwrite and staged_files_present(destination, staged_name):
                statuses[city_name] = f"skipped, already staged at {join_path(destination, staged_name)}"
            ...
            outputs = process_city_data(country_name, city_name, target_crs, working_dir, h3_root, output_dir)
...
def main(argv=None):
    parser = argparse.ArgumentParser(
        description=(
            "Extract and assemble city-only datasets (DEM, buildings, green spaces, soil, "
            "landcover) from H3 source folders for one or more cities in a country. Inputs "
            "are read from Google Cloud Storage by default. With --destination, results are "
            "staged per city and cities already staged are skipped."
        )
    )
    parser.add_argument(
        "country",
        help='Country name as it appears in the urban-areas shapefile, e.g. "United States"',
    )
    parser.add_argument(
        "city",
        nargs="+",
        help='One or more city names as they appear in the urban-areas shapefile, e.g. "Fort Wayne"',
    )
    parser.add_argument(
        "--working-dir",
        default=DEFAULT_WORKING_DIR,
        help=(
            "Directory (local path or gs:// URI) containing 01_urban_areas_simplified.shp "
            "and h3_cells_level_2.shp. (default: %(default)s)"
        ),
    )
    parser.add_argument(
        "--h3-root",
        ...
        help=(
            "Root (local path or gs:// URI) containing one folder per H3 index. "
            "(default: %(default)s)"
        ),
    )
    parser.add_argument(
        ...
        help=(
            "Local working directory; results are written to a <Country>_<City> "
            "subfolder here. (default: %(default)s)"
        ),
    )
    parser.add_argument(
        "--destination",
        default=None,
        help=(
            "Staging root (gs:// URI or local directory) that receives a <City>/ folder "
            "per city in the BETA_10_CITIES layout, e.g. "
            "gs://citycat-preprocessing/BETA_10_CITIES. Cities already staged there are "
    ...
    parser.add_argument(
        "--overwrite",
        action="store_true",
        help="Reprocess and re-stage cities that are already present at --destination.",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Only report which cities would be processed or skipped; do not process anything.",
    )
