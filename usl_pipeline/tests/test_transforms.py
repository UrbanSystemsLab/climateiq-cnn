...
from usl_pipeline import transforms


@mock.patch.object(transforms.storage, "Client", autospec=True)
def test_build_feature_matrix(mock_client):
    ...
    transforms.build_feature_matrix(
        functions_framework.CloudEvent(
            {"source": "test", "type": "event"},
            data={"bucket": "bucket", "name": "map/chunks/name.tar"},
        )
    )
    ...
    mock_client.assert_has_calls(
        [
            ...
            mock.call().bucket().blob("map/chunks/name.tar"),
            mock.call().bucket().blob().open("rb"),
            mock.call().bucket().blob("map/features/name.npy"),
