...
class FloodModelPredictor(Predictor):
    ...
    def load(self, artifacts_uri: str) -> None:
        ...
        loaded_model = tf.saved_model.load('concret_fn')
    ...
    def preprocess(self, input_data) -> dict:
        ...
        """
        # load jsnol local file and create a dict
    ...
    def predict_now(self, instances_dict):
        # Ensure n is a scalar of type int32
        instances_dict["n"] = tf.cast(tf.squeeze(instances_dict["n"]), tf.int32)
        
        # Prepare the input in the format expected by your model
        model_input = {
            "geospatial": instances_dict["geospatial"],
            "temporal": instances_dict["temporal"],
            "spatiotemporal": instances_dict["spatiotemporal"],
            "n": instances_dict["n"]
        }

        # Call the model's __call__ method with the correct input format
        prediction = self._model(model_input, training=False)
