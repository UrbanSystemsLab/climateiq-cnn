...
# Use your existing signatures
# _geospatial_dataset_signature()
# _temporal_dataset_signature(m_rainfall)
# _spatiotemporal_dataset_signature(n_flood_maps)
...
def download_dataset(
    ...
    - Sims whose study-area contains 'Prediction' (case-insensitive): temporal +
      features-only per split (no labels), keeping the SAME directory layout
      as Manhattan (train/val exist, but only geospatial/ is populated).
    """
    firestore_client = firestore_client or firestore.Client()
    storage_client = storage_client or storage.Client()
    ...
    for sim_name in sim_names:
        ...
        if is_prediction:
            # ---- temporal.npy ----
            # Use the *base* study-area (without _Prediction) to fetch temporal metadata
            # for the same rainfall file, so the vector matches Rainfall_Data_X.txt.
            ...
            # ---- geospatial features (no labels), duplicated to each requested split ----
            feature_metas = list(
            ...
            for split in dataset_splits:
                ...
                for fm in feature_metas:
                    ...
                    stem = pathlib.Path(fm["feature_matrix_path"]).stem
                    np.save(features_dir / f"{stem}.npy", arr)
                    total_downloaded += 1

            print(
                f"  ✓ [Prediction] Saved temporal.npy and {len(feature_metas)} feature chunks "
...
def load_dataset_cached(
    ...
    def gen():
        for sim_name in sim_names:
            ...
            t_tf = tf.tile(
                tf.reshape(t_tf, (-1, 1)), [1, m_rainfall]
            )  # [T, m_rainfall]
            t_tf = t_tf[:n_flood_maps, :]
            if int(t_tf.shape[0]) < n_flood_maps:
                t_tf = tf.concat(
                    [
                        t_tf,
                        tf.zeros(
                            (n_flood_maps - int(t_tf.shape[0]), m_rainfall), tf.float32
                        ),
                    ],
                    axis=0,
                )
            ...
            if include_labels and labels_dir.exists():
                label_map = {p.stem: p for p in labels_dir.glob("*.npy")}
            ...
            if sp_dir.exists():
                ...
                if sfiles:
                    mats = [
                        np.load(str(p)).astype(np.float32, copy=False) for p in sfiles
                    ]
                    mats = [
                        m if m.ndim == 3 else m[..., None] for m in mats
                    ]  # [H,W,1+]
                    ...
                    if sp_np.shape[0] > n_flood_maps:
                        ...
                        pad = np.zeros(
                            (n_flood_maps - sp_np.shape[0],) + sp_np.shape[1:],
                            np.float32,
                        )
            ...
            if sp is None:
                sp = tf.zeros(
                    (n_flood_maps, constants.MAP_HEIGHT, constants.MAP_WIDTH, 1),
                    tf.float32,
                )
            ...
            for i, fpath in enumerate(feat_files):
                ...
                if include_labels:
                    lp = label_map.get(fpath.stem)
                    if lp is None and strict_labels:
                        raise FileNotFoundError(
                            f"No label for chunk '{fpath.stem}' under {labels_dir}"
                        )
                    if lp is not None:
                        lab = np.load(str(lp)).astype(np.float32, copy=False)
                        # ---- collapse to single-channel (union over channels)
                        if lab.ndim == 3:
                            if lab.shape[2] == 1:
                                lab = lab[:, :, 0]
                            else:
                            ...
                            raise ValueError(
                                f"Unexpected label ndim={lab.ndim}; expected 2 or 3."
                            )
                    else:
                        lab = np.zeros((H, W), np.float32)

                    # pad/crop H,W and ensure shape
    ...
    output_sig = (
        (
            input_sig,
            tf.TensorSpec((constants.MAP_HEIGHT, constants.MAP_WIDTH), tf.float32),
        )
        if include_labels
        else input_sig
    )
...
def load_prediction_dataset_cached(
    ...
    def gen():
        for sim_name in sim_names:
            ...
            if t_vec.ndim > 1:
                t_vec = t_vec.reshape(-1)
            t_tf = tf.convert_to_tensor(t_vec, tf.float32)  # [T]
            t_tf = tf.tile(
                tf.reshape(t_tf, (-1, 1)), [1, m_rainfall]
            )  # [T, m_rainfall]
            t_tf = t_tf[:n_flood_maps, :]
            ...
            for i, fpath in enumerate(feat_files):
                ...
                if out.shape[2] > G:
                    ...
                    out = np.concatenate([out, pad], axis=-1)
                geo_tf = tf.convert_to_tensor(out, tf.float32)
                geo_tf = tf.ensure_shape(
                    geo_tf,
                    (constants.MAP_HEIGHT, constants.MAP_WIDTH, constants.GEO_FEATURES),
                )

                inputs = {"temporal": t_tf, "geospatial": geo_tf, "spatiotemporal": sp}
                meta = {
                    "feature_chunk": tf.convert_to_tensor(fpath.stem, tf.string),
    ...
    ds = tf.data.Dataset.from_generator(
        ...
        output_signature=(
            dict(
                geospatial=_geospatial_dataset_signature(),
                temporal=tf.TensorSpec(
                    (n_flood_maps, m_rainfall), tf.float32
                ),  # explicit
                spatiotemporal=_spatiotemporal_dataset_signature(n_flood_maps),
            ),
            dict(
