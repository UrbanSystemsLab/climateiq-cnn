...
def plot(
    ...
    if label is not None:
        yield plot_2d_timeseries(
            ...
            vmin=gt_vmin if gt_vmin is not None else sto_var_config.norm_vmin,
            vmax=gt_vmax if gt_vmax is not None else sto_var_config.norm_vmax,
            t_start=0.0,
            t_interval=0.5,
            normalize=normalize,
        )
    ...
    if pred is not None:
        yield plot_2d_timeseries(
            ...
            vmin=pred_vmin if pred_vmin is not None else sto_var_config.norm_vmin,
            vmax=pred_vmax if pred_vmax is not None else sto_var_config.norm_vmax,
            t_start=0.0,
            t_interval=0.5,
            normalize=normalize,
        )
    ...
    if label is not None and pred is not None:
        ...
        diff_range = max(abs(sto_var_config.norm_vmin), abs(sto_var_config.norm_vmax))
        yield plot_2d_timeseries(
            diff,
...
def plot_batch(
    ...
    """Plot a batch of AtmoML Examples with separate vmin/vmax for ground truth and prediction."""
