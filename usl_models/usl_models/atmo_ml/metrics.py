...
class OutputVarMeanSquaredError(keras.metrics.MeanMetricWrapper):
    ...
    def __init__(self, sto_var: vars.SpatiotemporalOutput, var_index: int, name=None, dtype=tf.float32):
        """
        Args:
            sto_var: the variable (e.g. T2, RH2, etc.)
            var_index: the index in the final output tensor (0-based)
        """
        ...
        name = name or f"mse_{sto_var.name.lower()}"
        super().__init__(fn=mse, name=name, dtype=dtype)
